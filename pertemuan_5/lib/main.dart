...
class Catatan {
  ...
  final String email;            // Pertahankan email dari Tugas sebelumnya
...
class _CatatanFormPageState extends State<CatatanFormPage> {
  ...
  void initState() {
    ...
    _emailCtrl = TextEditingController(text: ini?.email ?? '');
  ...
  Future<void> _simpan() async {
    ...
    try {
      ...
      if (_isEdit) {
        await DbHelper.instance.update(hasil);
      } else {
        await DbHelper.instance.insert(hasil);
      }

      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text(_isEdit ? 'Catatan diperbarui' : 'Catatan disimpan')),
      );
